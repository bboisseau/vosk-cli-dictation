...
try:
    from src.dictation_dbus import DictationDBusBridge
...
def main():
    ...
    dbus_event_queue = queue.Queue()
    stop_event = threading.Event()
    display_partials_event = threading.Event()

    processor = TextProcessor(config.current_lang)

    partial_cb = lambda text: dbus_event_queue.put({'type': 'partial', 'value': text})

    threads = [
        threading.Thread(target=recognition_thread, args=(recognizer, audio_queue, text_queue, stop_event, display_partials_event), kwargs={'partial_callback': partial_cb, 'flush_event': flush_event, 'flush_done_event': flush_done_event}),
        threading.Thread(target=audio_capture_thread, args=(stream, audio_queue, stop_event)),
        threading.Thread(target=ui_thread, args=(text_queue, control_queue, stop_event, processor, display_partials_event, hud_queue), kwargs={'dbus_queue': dbus_event_queue, 'auto_start': not _DBUS_AVAILABLE, 'flush_event': flush_event, 'flush_done_event': flush_done_event})
    ]
    # Only run pynput keyboard listener when D-Bus extension is not controlling input
    if not _DBUS_AVAILABLE:
        threads.append(threading.Thread(target=keyboard_listener_thread, args=(control_queue, stop_event)))

    if not _DBUS_AVAILABLE and run_hud_overlay and (os.environ.get('DISPLAY') or os.environ.get('WAYLAND_DISPLAY')):
        # Fallback tkinter HUD — only used when the GNOME Shell extension D-Bus bridge is absent
        threads.append(
    ...
    # Start D-Bus bridge so the GNOME Shell extension can connect
    if _DBUS_AVAILABLE:
        DictationDBusBridge(control_queue, dbus_event_queue).start()

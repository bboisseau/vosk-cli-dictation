...
const PROJECT_DIR = GLib.build_filenamev([GLib.get_home_dir(), 'Documents', 'Repos', 'vosk-cli-dictation']);
...
export default class VoskExtensionPreferences extends ExtensionPreferences {
    fillPreferencesWindow(window) {
        ...
        const gnomeShortcutGroup = new Adw.PreferencesGroup({
            title: this.gettext('GNOME Shell Shortcut'),
            description: this.gettext('Key combination that triggers the overlay directly from GNOME Shell'),
        });
        ...
        const formatAccel = (accel) => {
            ...
            try {
                ...
                if (ok) return Gtk.accelerator_get_label(keyval, mods);
            } catch (_) {}
            return accel;
        };

        const shortcutRow = new Adw.ActionRow({
            title: this.gettext('Shortcut'),
            subtitle: this.gettext('Click to record a new key combination'),
            activatable: true,
        });

        const accelLabel = new Gtk.Label({
            label: formatAccel(settings.get_strv('dictation-trigger')[0] ?? ''),
            valign: Gtk.Align.CENTER,
            css_classes: ['accelerator'],
        });
        shortcutRow.add_suffix(accelLabel);

        const clearBtn = new Gtk.Button({
            icon_name: 'edit-clear-symbolic',
            valign: Gtk.Align.CENTER,
            tooltip_text: this.gettext('Disable shortcut'),
            css_classes: ['flat', 'circular'],
        });
        clearBtn.connect('clicked', () => {
            settings.set_strv('dictation-trigger', []);
            accelLabel.label = 'Disabled';
        });
        ...
        const stopCapture = (accel) => {
            if (captureController) {
                shortcutRow.get_root().remove_controller(captureController);
                captureController = null;
            }
            capturing = false;
            if (accel) {
                settings.set_strv('dictation-trigger', [accel]);
                accelLabel.label = formatAccel(accel);
            } else {
                accelLabel.label = formatAccel(settings.get_strv('dictation-trigger')[0] ?? '');
            }
            shortcutRow.subtitle = this.gettext('Click to record a new key combination');
        };

        shortcutRow.connect('activated', () => {
            if (capturing) return;
            capturing = true;
            shortcutRow.subtitle = this.gettext('⌨  Press your shortcut now… (Esc to cancel)');
            accelLabel.label = '…';

            captureController = new Gtk.EventControllerKey();
            captureController.set_propagation_phase(Gtk.PropagationPhase.CAPTURE);
            captureController.connect('key-pressed', (_ctrl, keyval, _code, state) => {
                if (MODIFIER_KEYSYMS.has(keyval)) return false;
                if (keyval === Gdk.KEY_Escape) { stopCapture(null); return true; }
                const mask = state & Gtk.accelerator_get_default_mod_mask();
                const accel = Gtk.accelerator_name(keyval, mask);
                stopCapture(accel || null);
                return true;
            });
            shortcutRow.get_root().add_controller(captureController);
        });

        gnomeShortcutGroup.add(shortcutRow);
        page.add(gnomeShortcutGroup);
